...
地図作成(SLAM)の入出力は下記の図のようになっています。  
`/slam_toolboxノード`は、`/scanトピック`と`/tfトピック(odom〜base_scanフレーム)`をサブスクライブし、`/mapトピック`と`/tfトピック(map→odom)`を出力します。  
SLAMのアルゴリズムでは計算上`/tfトピック(map→base_footprint)`が得られますが、TFのルール上、親フレームが1つでなければならないことと、自己位置推定の際に後述する理由で、`(map→base_footprint)`と`(odom→base_footprint)`のベクトルの引き算により、`(map→odom)`を出力しています。
`/map_saverノード`は、`/mapトピック`をサブスクライブし、地図データとして`pgmファイル`と`yamlファイル`を出力しています。
...
slam_launch.pyでは、`map_saver_serverノード`と`slam_toolbox(online_async_launch)ノード`を起動しています。
