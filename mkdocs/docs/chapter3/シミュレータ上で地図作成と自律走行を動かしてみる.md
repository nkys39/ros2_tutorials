...
インストールした時点で地図作成と自律走行を動かすためのlaunchファイルは`/opt/ros/humble/share/[パッケージ名]/launch/ディレクトリ`にあります。  
動かすことはできますが、このままでは編集ができないので、動かすためのlaunchファイルを`lecture_nav2_bringupパッケージ`にコピーしてまとめてあります。  
...
```sh
# ジョイスティックがある場合
ros2 launch lecture_nav2_bringup joy_teleop_launch.py
# キーボードで操作する場合
ros2 run teleop_twist_keyboard teleop_twist_keyboard
```
ジョイスティックで操作の場合、1番ボタン(低速)または,2番ボタン(高速)を押しながら左スティックを倒した方向に移動します。    
キーコンフィグはteleop_twist_joyノードのパラメータで変更可能です
...
- map_serverパッケージには地図保存の方法が2種類提供されています
    ...
    - map_saver_serverノード
      ...
      # サービスコマンドで地図保存ができます
      ros2 service call /map_saver/save_map nav2_msgs/srv/SaveMap "{map_topic: map, map_url: /home/user/map, image_format: pgm, map_mode: trinary, free_thresh: 0.25, occupied_thresh: 0.65}"
      ```

上記のどちらかを実行すると、`~/map.yaml`と`~/map.pgm`が作成されます。  
この`pgmファイル`と`yamlファイル`がROS2での自律走行の地図になります。  
詳細については【5章】で説明しますが、`pgmファイル`が占有格子地図の各グリッドの値を画像ファイルとして持ち、`yamlファイル`が原点座標などの設定値を持っています。  
...
ros2 launch nav2_bringup rviz_launch.py
# 自己位置推定の起動
ros2 launch nav2_bringup localization_launch.py map:=/home/user/map.yaml use_sim_time:=True
```
